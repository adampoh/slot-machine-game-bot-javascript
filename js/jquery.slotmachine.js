...
(function init($, window, document, undefined) {
    ...
    class SlotMachine {
         ...
         */
        autoSlot() {
            winPercent = parseInt($("#winPercent").val());
            if (winPercent) {
                setAuto = Math.round(Math.random() * 100 - (100 - winPercent));
            } else {
                setAuto = -100;
            }
            futureValue = Math.round(Math.random() * 6);
            console.log(setAuto);
        }
